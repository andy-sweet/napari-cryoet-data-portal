...
def make_filter(
    filter_type: Union[
        Type[Dataset], Type[Run], Type[TomogramVoxelSpacing], Type[Tomogram]
    ],
    ids: Tuple[int, ...],
) -> Filter:
    if filter_type is Dataset:
        return DatasetFilter(ids=ids)
    elif filter_type is Run:
        return RunFilter(ids=ids)
    elif filter_type is TomogramVoxelSpacing:
        return SpacingFilter(ids=ids)
    elif filter_type is Tomogram:
        return TomogramFilter(ids=ids)
    else:
        raise RuntimeError("Entity type not supported: %s", filter_type)


def _ids_to_gql(
    id_field: GQLField, ids: Tuple[int, ...]
) -> Tuple[GQLExpression, ...]:
