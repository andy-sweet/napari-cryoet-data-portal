...
class UriWidget(QGroupBox):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._filter_ids_type.addItem(
            "Voxel Spacing IDs", TomogramVoxelSpacing
        )
    ...
    def _onConnectClicked(self) -> None:
        ...
        data_filter = make_filter(
            self._filter_ids_type.currentData(),
            ids=filter_ids,
        )
        logger.debug("UriWidget._onConnectClicked: %s, %s", uri, data_filter)
    ...
    def _connect(self, uri: str, data_filter: Filter) -> Tuple[str, Filter]:
        _ = Client(uri)
        return uri, data_filter

    def _onConnected(self, result: Tuple[str, Filter]) -> None:
        uri, data_filter = result
        logger.debug("UriWidget._onConnected: %s, %s", uri, data_filter)
        self._updateVisibility(True)
        self.connected.emit(uri, data_filter)
